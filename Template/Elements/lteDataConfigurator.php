...
class lteDataConfigurator extends lteTabs
{
     ...
     * 
     * {@inheritDoc}
     * @see \exface\AdminLteTemplate\Template\Elements\lteTabs::buildHtmlTabHeaders()
     */
...
HTML;
     ...
     * 
     * {@inheritDoc}
     * @see \exface\AdminLteTemplate\Template\Elements\lteTabs::buildHtmlTabBodies()
     */
    protected function buildHtmlTabBodies()
    {
        ...
        foreach ($widget->getWidgetConfigured()->getColumns() as $col) {
            if ($col->isHidden() && ! $this->getWorkbench()->context()->getScopeUser()->getUserCurrent()->isUserAdmin()) {
                continue;
            }
            $columns_html .= '
                                <li>
